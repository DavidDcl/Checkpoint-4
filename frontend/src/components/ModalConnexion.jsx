...
function ModalConnexion({ toggleModalConnexion, toggleModalInscritpion }) {
  ...
  const checkLoginForm = (obj) => {
    ...
    if (values.length) {
      if (values.length === 1) {
        message = `Complete ${values[0]}.`;
      } else {
        message = `Complete ${[...values]}.`;
      }
    }
    ...
    if (!errorMessage.length) {
      try {
        const user = await expressApi.post(`/signin`, {
          username: formDataConnexion.username,
  ...
  return (
    <div className="fixed inset-0 h-screen flex items-center justify-center bg-zinc-800 bg-opacity-70 z-20">
      <div className="bg-slate-500 rounded-xl w-80 flex flex-col">
        <div className="flex items-center justify-between  rounded-t-xl p-3">
          <h3 className="text-xl text-slate-200 font-bold">Connexion :</h3>
        ...
        <div className="p-3 flex flex-col gap-4">
          ...
          <button
            ...
            className="text-slate-300 text-sm underline cursor-pointer"
          >
